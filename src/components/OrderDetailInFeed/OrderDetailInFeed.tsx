...
import { IIngredient, IngredientInOrder, IOrder } from '../../services/types/interfaces';
...
const OrderDetailInFeed: FC = () => {
  ...
  let orderDate;
  if (date) orderDate = new Date(date).toLocaleString();
  ...
  for(let key in duplicates){
    ...
    if(obj) {
      obj.count = Number(duplicates[key]); //добавляем количество
  ...
  return <>
    {orders && (<div className={location.state === undefined ? style.container : undefined}>
      {location.state === undefined ? (
          <h3 className={style.title + " text text_type_digits-default"}>#{order?.number}</h3>)
